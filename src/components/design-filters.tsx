  ...
  return (
    <form className="filter-bar design-filter-bar" method="get" onSubmit={() => setExpanded(false)}>
      ...
      <section className="filter-options" id="design-filter-options" hidden={!expanded} aria-label="Design filters">
        ...
        <label><span>Karigar</span><select name="karigarId" defaultValue={values.karigarId ?? ""}><option value="">All karigars</option>{karigars.map((karigar) => <option key={karigar.id} value={karigar.id}>{karigar.name}</option>)}<option value="unassigned">Unassigned</option></select></label>
